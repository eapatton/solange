...
db.Mood.deleteMany({})
  .then((_result) => db.Posts.deleteMany())
  .then(() => db.Posts.create(posts))
  .then(() =>
...
// .then((_result) => db.users.deleteMany())
// .then(() => db.users.create(users))
// .then(() =>
//   // return a Promise.all
//   Promise.all(
//     // takes an array of promises
//     // callback of map returns a promise
//     posts.map((post) =>
//       db.User.find({ title: post.postTitles }).then((results) => {
//         mood.properties.users = results;
//         return db.Mood.create(mood.properties);
//       })
//     )
//   )
